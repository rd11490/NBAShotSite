...
class FourFactorsThreeYearAdderHandler extends Proxy[Seq[RealAdjustedFourFactors], String] {
  ...
  override def handle(
    ...
    if (auth.exists(FourFactorsThreeYearAdderHandler.verifyId)) {
      val message = input.body match {
        case Some(fourFactors) =>
          val success = PostgresClient.insertInto(NBATables.real_adjusted_four_factors_three_year, fourFactors)
...
object FourFactorsThreeYearAdderHandler {
  implicit val executionContext: ExecutionContext = ExecutionContext.global
