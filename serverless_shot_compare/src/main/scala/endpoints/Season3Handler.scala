...
class Season3Handler extends Proxy[String, SeasonsResponse] {
  ...
  override def handle(
    ...
    val response = Season3Handler.selectSeasons().map(v => SeasonsResponse(v.sorted.reverse)).await
    APIResponse.response(Some(response))
  }
}

object Season3Handler {
  implicit val executionContext: ExecutionContext = ExecutionContext.global
  def selectSeasons( /*IO*/ ): Future[Seq[String]] = {
    PostgresClient
      .selectSeasons(NBATables.seasons3)
  }
}
