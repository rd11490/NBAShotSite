...
class FourFactorsFiveYearHandler
  ...
  override def handle(
    ...
    val response = input.body.map(v => {
      ...
      FourFactorsFiveYearHandler.selectFourFactors(fourFactorsParams).await
    })
    APIResponse.response(response)
  }
}

object FourFactorsFiveYearHandler {
  implicit val executionContext: ExecutionContext = ExecutionContext.global
  ...
  def selectFourFactors(
    ...
    PostgresClient
      .selectFrom(
        NBATables.real_adjusted_four_factors_five_year,
