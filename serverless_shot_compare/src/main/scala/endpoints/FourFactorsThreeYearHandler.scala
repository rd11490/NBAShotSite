...
class FourFactorsThreeYearHandler
  ...
  override def handle(
    ...
    val response = input.body.map(v => {
      ...
      FourFactorsThreeYearHandler.selectFourFactors(fourFactorsParams).await
    })
    APIResponse.response(response)
  }
}

object FourFactorsThreeYearHandler {
  implicit val executionContext: ExecutionContext = ExecutionContext.global
  ...
  def selectFourFactors(
    ...
    PostgresClient
      .selectFrom(
        NBATables.real_adjusted_four_factors_three_year,
