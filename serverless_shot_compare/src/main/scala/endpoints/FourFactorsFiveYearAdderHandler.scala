...
class FourFactorsFiveYearAdderHandler extends Proxy[Seq[RealAdjustedFourFactors], String] {
  ...
  override def handle(
    ...
    if (auth.exists(FourFactorsFiveYearAdderHandler.verifyId)) {
      val message = input.body match {
        case Some(fourFactors) =>
          val success = PostgresClient.insertInto(NBATables.real_adjusted_four_factors_five_year, fourFactors)
...
object FourFactorsFiveYearAdderHandler {
  implicit val executionContext: ExecutionContext = ExecutionContext.global
