...
import com.rrdinsights.shotcompare.utils.storage.datamodel.{ DataModelUtils, PlayersOnCourt, RawPlayByPlayEvent }
...
object PlayersOnCourtDownloader {
  ...
  def writePlayersOnCourt(players: Seq[PlayersOnCourt]): Unit = {
    PostgresClient.createTable(NBATables.players_on_court)
    PostgresClient.insertInto(NBATables.players_on_court, players)
  }

  def writePlayersOnCourtAtPeriod(players: Seq[PlayersOnCourt]): Unit = {
    PostgresClient.createTable(NBATables.players_on_court_at_period)
    PostgresClient.insertInto(NBATables.players_on_court_at_period, players)
  }

  def readPlayersOnCourtAtPeriod(where: String*): Seq[PlayersOnCourt] = {
    PostgresClient.selectFromAndWait[PlayersOnCourt](
