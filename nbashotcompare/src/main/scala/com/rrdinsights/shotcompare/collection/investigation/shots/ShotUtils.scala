...
import com.rrdinsights.shotcompare.utils.storage.datamodel.{ RawShotData, ScoredShot, ShotWithPlayers }
...
object ShotUtils {
  ...
  def readShotsWithPlayers(where: String*): Seq[ShotWithPlayers] =
    PostgresClient.selectFromAndWait(NBATables.lineup_shots, ShotWithPlayers.apply, where: _*)

  def readShots(where: String*): Seq[RawShotData] =
    PostgresClient.selectFromAndWait(NBATables.raw_shot_data, RawShotData.apply, where: _*)

  def readScoredShots(where: String*): Seq[ScoredShot] =
    PostgresClient.selectFromAndWait(NBATables.team_scored_shots, ScoredShot.apply, where: _*)
