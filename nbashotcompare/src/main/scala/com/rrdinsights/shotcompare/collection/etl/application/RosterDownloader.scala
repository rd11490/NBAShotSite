...
import com.rrdinsights.shotcompare.utils.storage.datamodel.{ RosterCoach, RosterPlayer }
...
object RosterDownloader {
  ...
  private def writePlayerInfo(players: Seq[Player], season: String, dt: String): Unit = {
    PostgresClient.createTable(NBATables.roster_player)
    val rosterPlayers = players.map(RosterPlayer(_, season, dt))
    PostgresClient.insertInto(NBATables.roster_player, rosterPlayers)
  }

  def readPlayerInfo(where: Seq[String] = Seq.empty): Seq[RosterPlayer] = {
    PostgresClient.selectFromAndWait[RosterPlayer](
  ...
  private def writeCoachInfo(coaches: Seq[Coach], season: String, dt: String): Unit = {
    PostgresClient.createTable(NBATables.roster_coach)
    val rosterCoaches = coaches.map(RosterCoach(_, season, dt))
    PostgresClient.insertInto(NBATables.roster_coach, rosterCoaches)
  }
