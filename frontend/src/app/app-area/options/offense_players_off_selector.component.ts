...
@Component({
  selector: 'offensive-players-off-select',
  template: `
    <div>
      Offensive Players Off Court
...
export class OffensePlayersOffSelectorComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    OptionSelectors.selectOffensivePlayersOff(this.store, this._source).subscribe(players => {
      if (players != null) {
  ...
  selectPlayers(players: Array<PlayerId>) {
    this.store.dispatch(new OptionActions.SetOffensePlayersOff(players, this._source));
  }
