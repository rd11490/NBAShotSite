...
import {GetPlayers, GetSeasons3, GetTeams} from "../../actions/initial.action";
import {FourFactorsThreeYearSearch, SearchInProgress, StoreFourFactors} from "../../actions/search.action";
import {selectFourFactorsResponseSearchActions, selectPageLoaded3} from "../../selectors/initial.selectors";
...
@Component({
  ...
  template: `
    ...
    <div [hidden]="(this._done_loading | async)">
      ...
      <four-factors-3yr-options></four-factors-3yr-options>
      <div>
...
export class FourFactorsThreeYearComponent implements OnInit {
  ...
  constructor(private store: Store<State>,
    ...
    store.dispatch(new GetSeasons3());
    ...
    if (hash != null) {
      ...
      this.store.dispatch(new SetHash("-357354019")); //TODO Update when new season rolls around
      this.searchWithHash();
    }
  ...
  ngOnInit(): void {
    ...
    this._loading = selectPageLoaded3(this.store);
    this._done_loading = selectPageLoaded3(this.store).map(v => !v);
  ...
  search = (): void => {
    ...
    this.store.dispatch(new FourFactorsThreeYearSearch());
  };
